...
### PPO Agent (parallel branch)

A PPO agent with a topology-aware symmetric score map was developed in parallel on the [`elora-ruche` branch](https://github.com/helloelora/rl-quantum-circuit-routing/tree/elora-ruche). It shares the same 5-channel environment but uses an on-policy actor-critic instead of value-based learning.
...
PPO **beats SABRE by 12.9% on `linear_5`** (14.1 vs 16.2 SWAPs). On `grid_3x3`, learning rate annealing (3e-4 → 3e-5) eliminated late-training regression. Heavy_hex remained an open challenge: without prioritized experience replay, the on-policy PPO entropy stayed near-uniform on the 22-action space even with depth curriculum. This confirms that off-policy methods like D3QN+PER are better suited for combinatorial routing problems with large action spaces.

The PPO branch also contributes **anti-collapse engineering** (reverse-swap penalty, progressive same-edge penalty, no-progress termination, action-repeat logit penalty, symmetric score map) which prevents degenerate policies on this combinatorial action space.

<div align="center">

<img src="docs/Architecture.png" width="700" alt="5-channel CNN state representation and dueling architecture">

*5-channel state representation feeding the shared CNN backbone with the dueling head shown. The PPO agent reuses the same backbone with a symmetric score map policy head and a separate value head.*
